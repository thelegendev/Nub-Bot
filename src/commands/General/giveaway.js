...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        switch (sub) {
            ...
            interaction.editReply({ content: `Your **giveaway** has been started **successfully** in ${showchannel}!`, ephemeral: true })
           ...
           await interaction.reply({ content: `**Editing** your giveaway...`, ephemeral: true});
            ...
            .end(messageId1)
            .then(() => {
                interaction.editReply({ content: 'Your **giveaway** has been ended **successfuly!**', ephemeral: true});
            })
            .catch((err) => {
                interaction.editReply({ content: `An **error** occured!`, ephemeral: true});
            });
            ...
            if (!giveaway) return interaction.editReply({ content: `**Couldn't** find a **giveaway** with the **ID:** **${query}**.`, ephemeral: true});
            const messageId2 = interaction.options.getString('message-id');
            client.giveawayManager.reroll(messageId2).then(() => {
                interaction.editReply({ content: `Your **giveaway** has been rerolled **successfuly**!`});
            })
            .catch((err) => {
                interaction.editReply({ content: `An **error** occured!`, ephemeral: true});
            });
